...
class CasService {
  ...
  Future<CasSession> loginCas(
    ...
    if (errMsg.contains('密码') || errMsg.contains('用户名')) {
  ...
  Future<CasSession> loginJw(String username, String password) async {
    ...
    // Must follow redirects step by step so CookieJar is consulted
    var ssoUrl = jwSsoUrl;
    for (var i = 0; i < 10; i++) {
      ...
      if (resp.statusCode == 301 || resp.statusCode == 302) {
        ...
        ssoUrl = loc.startsWith('http') ? loc : Uri.parse(ssoUrl).resolve(loc).toString();
        continue;
      }

...
- Android 使用 `adjustResize`，同一可见页面树只能有一层 Scaffold 处理键盘 `viewInsets`：`HomePage` 外壳负责底部标签根页面，`AppPage(root: true)` 不得再次扣除；路由打开的非根页面由自己的 `AppPage` 处理。
- 聚焦输入框使用 Forui 默认的 20dp `scrollPadding`，只滚动到完整露出且与键盘保持合理距离的位置。禁止将其覆盖为零，也不要让嵌套 Scaffold 重复避让或手动平移整个页面制造过度跳动。
...
- [ ] 打开和关闭键盘时页面正确缩放/复位；聚焦输入框完整可见、与键盘约留 20dp，页面只滚动必要距离且底部导航不覆盖输入内容。
...
| 版本 | 日期 | 说明 |
| --- | --- | --- |
| 1.5.0 | 2026-08-16 | 键盘 inset 由可见页面树中的单层 Scaffold 处理；根标签页委托给 `HomePage` 外壳，消除嵌套 Scaffold 重复避让产生的大块空白。 |
| 1.4.0 | 2026-08-16 | 固定启用键盘避让；滚动表单使用 Forui 默认 20dp `scrollPadding` 将聚焦字段移动到键盘上方，删除零留白和禁止滚动定位规则。 |

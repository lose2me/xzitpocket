...
- 键盘出现时保持页面和输入框原位，不调整页面高度、不整体上移，也不自动滚动；Android 使用 `adjustNothing`，Flutter 页面忽略底部 `viewInsets`。
...
| 1.2.0 | 2026-08-16 | 键盘改为覆盖式显示且不触发布局位移；设置行内输入器允许紧凑显示；Tool 一卡通与电费摘要固定并排。 |
